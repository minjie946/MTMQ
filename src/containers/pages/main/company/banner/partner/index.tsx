/**
 * @description banner-partner-合伙人版
 * @author minjie
...
interface PartnerInformationProps extends BaseProps, FormComponentProps {
}

interface PartnerInformationState {
  errorMsg: string
...
@inject('mobxCommon')
@observer
class PartnerInformation extends RootComponent<PartnerInformationProps, PartnerInformationState> {
  // 弹出关闭的key 默认删除： 0
  handleModalKey:number = 0
  // 删除时的key
  removeKey: number = -1

  constructor (props:PartnerInformationProps) {
    super(props)
    this.state = {
      errorMsg: '',
      visibleModal: false,
  ...
  componentDidUpdate (prevProps:any) {
    ...
    if (defaultActiveKeyPre !== defaultActiveKey && defaultActiveKey === 'partner') {
      this.loadingList()
    }
  }
  ...
  loadingList = () => {
    ...
    this.axios.request(this.api.bannerQuery, {
      ...
      let bannerAry:any = res.data.map((el:any, key:number) => {
        ...
        obj[`postAry${key}`] = []
        this.organizeChange(key, el.organize || [])
        return obj
      })
  ...
  /** 根据组织查询 岗位的信息 */
  organizeChange = (key:number, value:any) => {
                  ...
                  <Col span={9}>
                    <Form.Item label="图片">
                      {getFieldDecorator('sowingMapImgPath' + key, {
                        ...
                        <UploadImage imgURLs={el[`sowingMapImgURL${key}`]} proportion={{ width: 750, height: 292, company: 'px' }}/>
                      )}
                    </Form.Item>
                  </Col>
                </Row>
...
export default Form.create<PartnerInformationProps>()(PartnerInformation)
